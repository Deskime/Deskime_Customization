...
def install_fonts():
    """Install all .ttf and .otf fonts in the core/fonts directory."""
    fonts_dir = Path('core/fonts')
...
def open_selector():
    """Open selector.pyw."""
    if os.path.exists('selector.pyw'):
        subprocess.Popen(['pythonw', 'selector.pyw'])
        time.sleep(2.2)  # Delay for users to read the message
    else:
        print_error("Error: selector.pyw not found. Please ensure it is in the current directory.")
...
def create_shortcut():
    """Create a shortcut for main.pyw in the startup folder."""
    startup_folder = Path(os.getenv('APPDATA')) / 'Microsoft' / 'Windows' / 'Start Menu' / 'Programs' / 'Startup'
    shortcut_path = startup_folder / 'main.lnk'
    target = Path(os.getcwd()) / 'main.pyw'

    # Install pywin32 for creating a proper shortcut
    install_package('pywin32')

    import win32com.client  # Import after installation

    # Create a Windows shortcut (.lnk)
    shell = win32com.client.Dispatch('WScript.Shell')
    shortcut = shell.CreateShortCut(str(shortcut_path))
    shortcut.TargetPath = str(target)
    shortcut.WorkingDirectory = str(Path(os.getcwd()))
    shortcut.save()

    print_success("Startup mode enabled.")
...
if __name__ == "__main__":
    ...
    print_waiting("Creating shortcut for startup...")  # Display waiting message
    create_shortcut()      # Create a shortcut for main.pyw
    print_success("Congrats! Deskime is installed!")  # Display success message
    open_selector()        # Open selector.pyw
    time.sleep(2)
