...
class WebWallpaper(QMainWindow):
    def __init__(self):
        super().__init__()
        # Get the current directory and construct the file URL
        current_dir = os.getcwd()
        if os.path.exists("settings.json"):
            with open("settings.json", "r") as f:
                settings = json.load(f)
                if settings.get("default_template") == True:
                    self.url = QUrl.fromLocalFile(os.path.join(current_dir, "apps","parallax-star-background-in-css", "dist", "index.html"))
                elif settings.get("default_template") == False:
                    self.url = QUrl(settings.get("custom_link"))
                else:
